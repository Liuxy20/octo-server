...
// TestRepro484_ProdTrace_SpacelessGroupShownInEverySpace reproduces the group
// path: a group whose row in `group` has an EMPTY space_id. The server cannot
// attribute it to any Space, so the GROUP branch of decideConvKeepInSpace
// (space_filter.go:274-303) fails open — resolved spaceID == "" → `return true` —
// and returns it for EVERY Space query. That empty conversation-level space_id is
// what drives a client SpaceFilter to fail open and render the group in every
// Space; the client cannot self-correct because the conversation carries no
// space_id to match on.
//
// Discriminator: the spaceless group appears in BOTH non-default Spaces, while a
// group carrying a real space_id is correctly isolated to its own Space.
func TestRepro484_ProdTrace_SpacelessGroupShownInEverySpace(t *testing.T) {
	s, ctx := reproSetup(t)
	const (
		groupSpaceless = "grp_spaceless_x" // group.space_id = '' → unresolvable
		groupInB       = "grp_in_spaceB"
		groupInC       = "grp_in_spaceC"
	)
	reproSeedGroup(t, ctx, groupSpaceless, "")
	reproSeedGroup(t, ctx, groupInB, reproSpaceB)
	reproSeedGroup(t, ctx, groupInC, reproSpaceC)
	// The user is a member of all three; membership is a prerequisite orthogonal
	// to the Space filter this test exercises.
	for _, g := range []string{groupSpaceless, groupInB, groupInC} {
		reproSeedGroupMember(t, ctx, g)
	}

	reproIMConvs = []*config.SyncUserConversationResp{
		reproGroupConv(groupSpaceless, "hi-spaceless"),
		reproGroupConv(groupInB, "hi-B"),
		reproGroupConv(groupInC, "hi-C"),
	}

	inB := reproCallConvSync(t, s, reproSpaceB)
	inC := reproCallConvSync(t, s, reproSpaceC)

	// The spaceless group fails open into BOTH Spaces.
	assert.True(t, reproContains(inB, groupSpaceless), "spaceless group shown in spaceB (fail-open)")
	assert.True(t, reproContains(inC, groupSpaceless),
		"spaceless group ALSO shown in spaceC (fail-open) -> the same group crosses Spaces")

	// Groups WITH a real space_id are isolated correctly (contrast).
	assert.True(t, reproContains(inB, groupInB), "spaceB group shown in spaceB")
	assert.False(t, reproContains(inC, groupInB), "spaceB group NOT shown in spaceC")
	assert.True(t, reproContains(inC, groupInC), "spaceC group shown in spaceC")
	assert.False(t, reproContains(inB, groupInC), "spaceC group NOT shown in spaceB")
}

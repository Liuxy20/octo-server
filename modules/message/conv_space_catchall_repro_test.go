...
//
// Self-contained: helpers are cs-prefixed so this file never collides with the
// #484 branch's repro harness when both land on main.
//
...
const csWebhookSecret = "cs-webhook-secret-484"
...
var (
	csIMOnce  sync.Once
	csIMSrv   *httptest.Server
	csIMConvs []*config.SyncUserConversationResp
)

func csFakeIM() *httptest.Server {
	csIMOnce.Do(func() {
		csIMSrv = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func csSetup(t *testing.T) (*server.Server, *config.Context) {
	...
	t.Setenv("TS_WEBHOOK_SECRET_KEY", csWebhookSecret)
...
func csIngestDM(t *testing.T, s *server.Server, peer, spaceID string, seq uint32) {
	...
	mac := hmac.New(sha256.New, []byte(csWebhookSecret))
	mac.Write([]byte(body))
